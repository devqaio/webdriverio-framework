...
class ReportBackupManager {
    ...
    async backup() {
        ...
        try {
            ...
            // Generate index of all backups (isolated — failure should not invalidate backup)
            try {
                await this._generateBackupIndex();
            } catch (indexErr) {
                logger.warn(`Failed to generate backup index: ${indexErr.message}`);
            }

            // Prune old backups (isolated — failure should not invalidate backup)
            try {
                await this._pruneOldBackups();
            } catch (pruneErr) {
                logger.warn(`Failed to prune old backups: ${pruneErr.message}`);
            }
    ...
    async _generateBackupIndex() {
        ...
        const rows = backups
            .map(
                (b) =>
                    `<tr><td><a href="${escapeHtml(b.name)}/index.html">${escapeHtml(b.name)}</a></td><td>${escapeHtml(b.date.toLocaleString())}</td></tr>`,
            )
            .join('\n');

        const safeProjectName = escapeHtml(this.projectName);

        const html = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>${safeProjectName} — Test Report Archive</title>
    <style>
...
<body>
    <h1>📊 ${safeProjectName} — Test Report Archive</h1>

...
class DataDrivenManager {
     ...
     * Uses recursive traversal instead of JSON roundtrip to prevent
     * issues with special characters in values.
     *
    ...
    _rebuildCache() {
        ...
        for (const [key, value] of this._stores) {
            if (typeof value === 'object' && !Array.isArray(value)) {
                // Warn on top-level key collisions that would silently overwrite data
                for (const subKey of Object.keys(value)) {
                    if (this._cache.hasOwnProperty(subKey)) {
                        logger.warn(`Key collision: "${subKey}" already exists in cache and will be overwritten by store "${key}"`);
                    }
                }
                ...
                if (this._cache.hasOwnProperty(key)) {
                    logger.warn(`Key collision: "${key}" already exists in cache and will be overwritten`);
                }

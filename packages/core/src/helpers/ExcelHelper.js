...
/** Lazy-load xlsx to avoid hard failure when not installed. Cached after first load. */
let _xlsx = null;
function getXlsx() {
...
/** Simple LRU cache for workbooks to avoid re-reading on every operation. */
const _workbookCache = new Map();
const WORKBOOK_CACHE_MAX = 10;

function _getCachedWorkbook(absPath, opts = {}) {
    const XLSX = getXlsx();
    const mtime = fs.statSync(absPath).mtimeMs;
    const cached = _workbookCache.get(absPath);
    if (cached && cached.mtime === mtime) {
        return cached.workbook;
    }
    const workbook = XLSX.readFile(absPath, { cellDates: true, ...opts });
    // Evict oldest entry if cache is full
    if (_workbookCache.size >= WORKBOOK_CACHE_MAX) {
        const firstKey = _workbookCache.keys().next().value;
        _workbookCache.delete(firstKey);
    }
    _workbookCache.set(absPath, { workbook, mtime });
    return workbook;
}

/** Invalidate workbook cache for a specific file (after writes). */
function _invalidateCache(absPath) {
...
class ExcelHelper {
    ...
    static readWorkbook(filePath) {
        ...
        const workbook = _getCachedWorkbook(absPath);
        const result = {};
    ...
    static updateCell(filePath, sheetName, cellAddress, value) {
        ...
        // Auto-detect cell type to preserve numbers and booleans

 ...
 * ═══════════════════════════════════════════════════════════════════════
 *
 * Extends BasePage with mobile-specific capabilities for iOS and Android
 * testing via Appium.  Includes:
 *
...
class MobileBasePage extends BasePage {
    constructor() {
        ...
        // Disable iframe auto-resolve on mobile (irrelevant for native)
     ...
     * @param {number} [percentage=0.75]  How much of the screen to swipe
     */
    ...
    async pinch(element) {
        ...
        // Two-finger pinch: both fingers move towards centre
     ...
     * @returns {Promise<string>}  'PORTRAIT' | 'LANDSCAPE'
     */
    ...
    /**
     * Reset the app by terminating and re-activating it (Appium 2.x compatible).
     * Clears app state by removing and reinstalling if `fullReset` is true.
     *
     * @param {string} [bundleId]  App bundle/package ID (reads from capabilities if omitted)
     * @param {boolean} [fullReset=false]  Remove and reinstall instead of terminate/activate
     */
    ...
    /**
     * Close (terminate) the current app session (Appium 2.x compatible).
    ...
    /**
     * Launch / re-launch the app under test (Appium 2.x compatible).
     * @param {string} [bundleId]  App bundle/package ID
     */
     ...
     * @param {string} appPath  Local path or URL to the .apk/.ipa
     */
     ...
     * @param {string} bundleId  e.g. 'com.example.app'
     */
    ...
    /**
     * Activate (bring to foreground) an existing app.
     * @param {string} bundleId
     */
    ...
    /**
     * Check if the software keyboard is visible.
     * @returns {Promise<boolean>}
     */
    ...
    /**
     * Lock the device.
     * @param {number} [seconds=0]  0 = indefinitely
     */
    ...
    /**
     * Press a hardware key (Appium 2.x compatible).
     * Uses `mobile: pressKey` for Android, falls back to `mobile: performIoHidEvent` for iOS.
     *
    ...
    async pressHardwareKey(keyCode) {
        if (this._isAndroid()) {
            ...
            // iOS: use page source navigation or custom mobile command
     ...
     * @param {string} url  e.g. 'myapp://product/123'
     */
     ...
     * @param {{ latitude: number, longitude: number, altitude?: number }} location
     */
     ...
     */
    async toggleData() {
        await browser.toggleData();
    }

    // ─── Notifications / System ───────────────────────────────

    /**
     * Open the notifications shade (Android).
     */
    ...
    /**
     * Get the current activity name (Android).
     * @returns {Promise<string>}
     */
    ...
    /**
     * Get the current package name (Android).
     * @returns {Promise<string>}
     */
    ...
    /**
     * Wait for an Android activity to load.
    ...
    /**
     * Wait until a native alert is present.
     */
    ...
    /**
     * Accept a native system alert / permission dialog.
     */
    async acceptAlert() {
        try {
            await browser.acceptAlert();
        } catch (err) {
            this.logger.warn(`acceptAlert: ${err.message}`);
        }
    }

    /**
     * Dismiss a native system alert / permission dialog.
     */
    async dismissAlert() {
        try {
            await browser.dismissAlert();
        } catch (err) {
            this.logger.warn(`dismissAlert: ${err.message}`);
        }
    }
    ...
    /**
     * Synchronous Android platform check using cached capabilities.
     * @returns {boolean}
     * @private
     */
    _isAndroid() {
    ...
    /**
     * Extract the app bundle/package ID from current capabilities.
     * @returns {Promise<string>}
     * @private
     */
    ...
    /**
     * Get the app path from capabilities (used for fullReset reinstall).
     * @returns {string}
     * @private
     */
    _getAppPath() {
